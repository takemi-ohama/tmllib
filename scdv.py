...
class SparseCompositeDocumentVectors:
    def __init__(self, model, num_clusters, embedding_dim):
        self.min_no = 0
        self.max_no = 0
        self.prob_wordvecs = {}
        self.model = model
    ...
    def get_probability_word_vectors(self, corpus):
        ...
        for word in word_centroid_map:
            ...
            for index in range(self.num_clusters):
                try:
                    self.prob_wordvecs[word][index*self.num_features:(index+1)*self.num_features] =                         self.model[word] * word_centroid_prob_map[word][index] * word_idf_dict[word]
                except:
                    continue
