...
class DB:
    ...
    def write_gbq(self, df, tablename, table_schema=None):
        df.to_gbq(tablename, project_id=self.project_id, if_exists='replace',
                  credentials=self.cred(), table_schema=table_schema)
        return df
    ...
    def cred(self):
        ...
        if self.account_type == 'file':
            ...
            json_key = json.loads(boto3.client('ssm').get_parameter(
            ...
            self._cred = service_account.Credentials.from_service_account_file(os.getenv('GOOGLE_APPLICATION_CREDENTIALS'))
        return self._cred
